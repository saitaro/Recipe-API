...
class PrivateTagsApiTests(TestCase):
    ...
    def test_retrieve_tags(self):
        """Test that the tags get retrieved."""
    ...
    def test_tags_limited_to_user(self):
        """Test that the tags received are for the authenticated user only."""
        user2 = get_user_model().objects.create_user(email='other@google.com',
                                                     password='otherpass')
        Tag.objects.create(name='Fruit', user=user2)                                                     
        tag = Tag.objects.create(name='Pizza', user=self.user)
    ...
    def test_retrieve_tags_assigned_to_recipes(self):
        ...
        recipe = Recipe.objects.create(title='French-style meat',
                                       time=55,
                                       price=70,
                                       user=self.user)
